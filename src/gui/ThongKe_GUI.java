...
public class ThongKe_GUI extends JPanel implements ActionListener {
	...
	private JComboBox<String> comboBox_ChonCa;
	...
	// === CÁC BIẾN MỚI CHO TAB "THỐNG KÊ VÉ" ===
	private JPanel jp_tkVe;
	private JPanel jp_thoiGian_Ve;
	private JPanel jp_header_Ve;
	private JPanel jp_content_Ve;
	private JLabel lbl_sltv_Ve;
	private JLabel lbl_slvb_Ve;
	private ChartPanel chartPanel_Ve;
	private String tkVe_ngayBatDau;
	private String tkVe_ngayKetThuc;
	private JDateChooser dateChooser_TKVe_batDau;
	private JDateChooser dateChooser_TKVe_ketThuc;
	private JPanel jp_thongKe_Ve;
	private JPanel jp_ketQua_Ve;
	private JButton btnXem_TKVe;
	private JButton btn_XuatFile_TheoVe;
	// ==========================================
	...
	public ThongKe_GUI(TrangChu_GUI trangChu) {
		...
		// === THAY ĐỔI TÊN TAB 1 ===
		tabbedPane.addTab("Thống kê theo ca", null, jp_tkdttheoca, null);
		...
		Image scaledDoanhThuTheoCa = doanhThuTheoCaIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay
																													// đổi
																													// kích
																													// thước
																													// logo
		...
		Image scaledSLTVTC = sltvtcIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi kích thước
																									// logo
		jp_ketQuaTheoCa.setLayout(null);
		...
		Image scaledSLVBTC = slvbtcIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi kích thước
																									// logo
		jp_ketQuaTheoCa.setLayout(null);
		...
		// === THAY ĐỔI TÊN TAB 2 ===
		tabbedPane.addTab("Thống kê doanh thu (theo thời gian)", null, jp_tkdt, null);
		...
		Image scaledDoanhThu = doanhThuIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi kích
																										// thước logo
		...
		Image scaledSLTV = sltvIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi kích thước
																								// logo
		jp_ketQua.setLayout(null);
		...
		Image scaledSLVB = slvbIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi kích thước
																								// logo
		jp_ketQua.setLayout(null);
		...
		btn_XuatFile_TheoDoanhThu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (dateChooser_TKDT_batDau.getDate() == null || dateChooser_TKDT_ketThuc.getDate() == null) {
					JOptionPane.showMessageDialog(null, "Vui lòng chọn ngày bắt đầu và ngày kết thúc!", "Thông báo",
				...
				if (data_TheoDoanhThu == null || data_TheoDoanhThu.isEmpty()) {
					JOptionPane.showMessageDialog(null, "Không có dữ liệu để xuất Excel!", "Thông báo",
							JOptionPane.WARNING_MESSAGE);
					return;
				}
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "Lỗi khi xuất file Excel: " + ex.getMessage(), "Lỗi",
		...
		// === TAB MỚI: THỐNG KÊ VÉ ===
		jp_tkVe = new JPanel();
		jp_tkVe.setBackground(SystemColor.textHighlightText);
		tabbedPane.addTab("Thống kê vé", null, jp_tkVe, null);
		jp_tkVe.setLayout(null);

		// JPane thời gian (Vé)
		jp_thoiGian_Ve = new JPanel();
		jp_thoiGian_Ve.setBackground(Color.WHITE);
		jp_thoiGian_Ve.setBounds(28, 10, 374, 115);
		jp_tkVe.add(jp_thoiGian_Ve);
		jp_thoiGian_Ve.setLayout(null);

		// JPane header (Vé)
		jp_header_Ve = new JPanel();
		jp_header_Ve.setBackground(new Color(51, 102, 153));
		jp_header_Ve.setBounds(0, 0, 374, 35);
		jp_thoiGian_Ve.add(jp_header_Ve);
		jp_header_Ve.setLayout(null);

		// JLabel tiêu đề (Vé)
		JLabel lblTieuDe_Ve = new JLabel("Thời gian");
		lblTieuDe_Ve.setHorizontalAlignment(SwingConstants.CENTER);
		lblTieuDe_Ve.setForeground(new Color(255, 255, 255));
		lblTieuDe_Ve.setFont(new Font("Tahoma", Font.BOLD, 18));
		lblTieuDe_Ve.setBounds(10, 0, 95, 35);
		jp_header_Ve.add(lblTieuDe_Ve);

		btnXem_TKVe = new RoundedButton("Xem", 15);
		btnXem_TKVe.setBackground(SystemColor.activeCaptionBorder);
		btnXem_TKVe.setForeground(SystemColor.desktop);
		btnXem_TKVe.setBounds(279, 10, 85, 21);
		jp_header_Ve.add(btnXem_TKVe);

		// JPane content (Vé)
		jp_content_Ve = new JPanel();
		jp_content_Ve.setBackground(SystemColor.controlHighlight);
		jp_content_Ve.setBounds(0, 36, 374, 79);
		jp_thoiGian_Ve.add(jp_content_Ve);
		jp_content_Ve.setLayout(null);

		// Ngày bắt đầu (Vé)
		dateChooser_TKVe_batDau = new JDateChooser();
		dateChooser_TKVe_batDau.setDateFormatString("dd/MM/yyyy");
		dateChooser_TKVe_batDau.setBounds(32, 5, 301, 29);
		jp_content_Ve.add(dateChooser_TKVe_batDau);
		dateChooser_TKVe_batDau.getDateEditor().addPropertyChangeListener("date", evt -> {
			Date selectedDate = dateChooser_TKVe_batDau.getDate();
			if (selectedDate != null) {
				SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
				tkVe_ngayBatDau = dateFormat.format(selectedDate);
			} else {
				tkVe_ngayBatDau = "";
			}
		});
		...
		// Ngày kết thúc (Vé)
		dateChooser_TKVe_ketThuc = new JDateChooser();
		dateChooser_TKVe_ketThuc.setDateFormatString("dd/MM/yyyy");
		dateChooser_TKVe_ketThuc.setBounds(32, 40, 301, 29);
		jp_content_Ve.add(dateChooser_TKVe_ketThuc);
		dateChooser_TKVe_ketThuc.getDateEditor().addPropertyChangeListener("date", evt -> {
			Date selectedDate = dateChooser_TKVe_ketThuc.getDate();
			if (selectedDate != null) {
				SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
				tkVe_ngayKetThuc = dateFormat.format(selectedDate);
			} else {
				tkVe_ngayKetThuc = "";
			}
		});

		// JPane chứa kết quả (Vé)
		jp_ketQua_Ve = new JPanel();
		jp_ketQua_Ve.setBackground(SystemColor.controlHighlight);
		jp_ketQua_Ve.setBounds(412, 10, 928, 115);
		jp_tkVe.add(jp_ketQua_Ve);
		jp_ketQua_Ve.setLayout(null);

		// Icon số lượng trả vé (Vé)
		ImageIcon sltvIcon_Ve = new ImageIcon(getClass().getResource("/img/money-back_1.png"));
		Image scaledSLTV_Ve = sltvIcon_Ve.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH);
		JLabel sltvIconLabel_Ve = new JLabel(new ImageIcon(scaledSLTV_Ve));
		sltvIconLabel_Ve.setBounds(200, 41, 40, 40); // Điều chỉnh vị trí
		jp_ketQua_Ve.add(sltvIconLabel_Ve);

		lbl_sltv_Ve = new JLabel();
		lbl_sltv_Ve.setFont(new Font("Tahoma", Font.PLAIN, 17));
		lbl_sltv_Ve.setHorizontalAlignment(SwingConstants.CENTER);
		lbl_sltv_Ve.setBounds(250, 41, 114, 20); // Điều chỉnh vị trí
		jp_ketQua_Ve.add(lbl_sltv_Ve);

		JLabel lbl_titleSLTV_Ve = new JLabel("Số lượng trả vé");
		lbl_titleSLTV_Ve.setBackground(SystemColor.windowBorder);
		lbl_titleSLTV_Ve.setForeground(new Color(105, 105, 105));
		lbl_titleSLTV_Ve.setFont(new Font("Tahoma", Font.PLAIN, 15));
		lbl_titleSLTV_Ve.setBounds(260, 61, 104, 20); // Điều chỉnh vị trí
		jp_ketQua_Ve.add(lbl_titleSLTV_Ve);

		// Icon số lượng vé bán (Vé)
		ImageIcon slvbIcon_Ve = new ImageIcon(getClass().getResource("/img/loan_1.png"));
		Image scaledSLVB_Ve = slvbIcon_Ve.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH);
		JLabel slvbIconLabel_Ve = new JLabel(new ImageIcon(scaledSLVB_Ve));
		slvbIconLabel_Ve.setBounds(470, 41, 40, 40); // Điều chỉnh vị trí
		jp_ketQua_Ve.add(slvbIconLabel_Ve);

		lbl_slvb_Ve = new JLabel();
		lbl_slvb_Ve.setFont(new Font("Tahoma", Font.PLAIN, 17));
		lbl_slvb_Ve.setHorizontalAlignment(SwingConstants.CENTER);
		lbl_slvb_Ve.setBounds(520, 41, 114, 20); // Điều chỉnh vị trí
		jp_ketQua_Ve.add(lbl_slvb_Ve);

		JLabel lbl_titleSLVB_Ve = new JLabel("Số lượng vé bán");
		lbl_titleSLVB_Ve.setBackground(SystemColor.windowBorder);
		lbl_titleSLVB_Ve.setForeground(new Color(105, 105, 105));
		lbl_titleSLVB_Ve.setFont(new Font("Tahoma", Font.PLAIN, 15));
		lbl_titleSLVB_Ve.setBounds(530, 61, 126, 20); // Điều chỉnh vị trí
		jp_ketQua_Ve.add(lbl_titleSLVB_Ve);

		btn_XuatFile_TheoVe = new RoundedButton("Xuất File", 15);
		btn_XuatFile_TheoVe.setForeground(new Color(255, 255, 255));
		btn_XuatFile_TheoVe.setBackground(new Color(51, 102, 153));
		btn_XuatFile_TheoVe.setBounds(809, 10, 96, 33);
		jp_ketQua_Ve.add(btn_XuatFile_TheoVe);
		btn_XuatFile_TheoVe.setFont(new Font("Tahoma", Font.PLAIN, 15));
		btn_XuatFile_TheoVe.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// (Bạn có thể sao chép logic từ btn_XuatFile_TheoDoanhThu và tùy chỉnh lại)
				// Tạm thời chỉ thông báo
				JOptionPane.showMessageDialog(null, "Chức năng xuất file cho Thống kê vé chưa được cài đặt.");
			}
		});

		// JPane chứa JFreeChat (Vé)
		jp_thongKe_Ve = new JPanel();
		jp_thongKe_Ve.setBounds(28, 133, 1311, 308);
		jp_thongKe_Ve.setBackground(SystemColor.controlHighlight);
		jp_tkVe.add(jp_thongKe_Ve);
		jp_thongKe_Ve.setLayout(null);
		...
		Image scaledDown1 = downIcon1.getImage().getScaledInstance(20, 20, Image.SCALE_SMOOTH); // Thay đổi kích thước
																								// logo
		...
		Image scaledDoanhThuCT = doanhThuCTIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi
																											// kích
																											// thước
																											// logo
		jp_ketQuaTheoCa.setLayout(null);
		...
		btn_XuatFile_TheoCa.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				////////////////////////////// Kiểm tra VaoCa và ketCa có null hay
				// không///////////////
				...
				NhanVien nv = dsNV.getNhanVienTheoTenNV(trangChu.lbl_ThongTinNV.getText());
				// Lấy danh sách hóa đơn cần xuất
				ArrayList<HoaDon> listHD = dsHD.getHoaDonTheoMaNV(nv.getMaNV()); // Thay thế `dsHD.getHoaDonTheoCa()`
																					// bằng cách lấy danh sách hóa đơn
																					// phù hợp
				if (listHD == null) {
					return;
				}

				data_TheoCa = new ArrayList<Ve>(); // Khởi tạo ArrayList
				...
				for (HoaDon hd : listHD) {
					...
					if (cthd != null) {
						...
						// Lấy danh sách vé và thêm vào data_TheoCa
						ArrayList<Ve> veTheoChiTiet = dsVe.getDsVeTheoMaChiTiet(cthd.getMaChiTiet());
						if (veTheoChiTiet != null) {
							data_TheoCa.addAll(veTheoChiTiet);
						}
					} else {
						System.out.println(
								"Chi tiết hóa đơn không tồn tại cho mã chi tiết: " + hd.getChiTiet().getMaChiTiet());
					}
				}
				...
				// Gọi phương thức xuất file Excel
				try {
					xuatExcelTheoCa(data_TheoCa, nv, thoiGianBatDauCa, thoiGianKetThucCa, lbl_doanhThuTheoCa,
							lbl_slvbtc, lbl_sltvtc);
					JOptionPane.showMessageDialog(null, "Xuất file Excel thành công!", "Thông báo",
							JOptionPane.INFORMATION_MESSAGE);
				} catch (IOException | java.io.IOException ex) {
					JOptionPane.showMessageDialog(null, "Lỗi khi xuất file Excel: " + ex.getMessage(), "Lỗi",
		...
		Image scaledSLTVCT = sltvctIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi kích thước
																									// logo
		jp_ketQuaCT.setLayout(null);
		...
		Image scaledSLVBCT = slvbctIcon.getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH); // Thay đổi kích thước
																									// logo
		jp_ketQuaCT.setLayout(null);
		...
		btnXem_TKVe.addActionListener(this); // === THÊM ACTION LISTENER CHO NÚT MỚI ===
	...
	public void actionPerformed(ActionEvent e) {
		...
		// === THÊM LOGIC CHO NÚT XEM CỦA TAB VÉ ===
		if (o.equals(btnXem_TKVe)) {
			updateKetQuaThongKeVe();
			createJFreeChartThongKeVe();
		}
		// =======================================
	...
	public void updateKetQuaThongKeTheoCa(TrangChu_GUI trangChu) {
		...
		for (HoaDon hd : listHD) {
			LocalDateTime thoiGianHoaDon = hd.getNgayLapHoaDon();
			if (hd.getDaHoanVe() && !thoiGianHoaDon.isBefore(thoiGianBatDauCa)
					&& !thoiGianHoaDon.isAfter(thoiGianKetThucCa)) {
				ChiTietHoaDon cthd = dsCTHD.getCTHDTheoMaChiTiet(hd.getChiTiet().getMaChiTiet());
				if (cthd != null) {
	...
	// === HÀM MỚI: TRUY VẤN THÔNG TIN CHO TAB VÉ ===
	public void updateKetQuaThongKeVe() {
		dsHD.reset();
		dsVe.reset();
		dsCTHD.reset();

		if (dateChooser_TKVe_batDau.getDate() == null || dateChooser_TKVe_ketThuc.getDate() == null) {
			JOptionPane.showMessageDialog(null, "Vui lòng chọn ngày bắt đầu và ngày kết thúc.");
			return;
		}

		SimpleDateFormat sqlDateFormat = new SimpleDateFormat("yyyy-MM-dd");
		String formattedNgayBatDau = sqlDateFormat.format(dateChooser_TKVe_batDau.getDate());
		String formattedNgayKetThuc = sqlDateFormat.format(dateChooser_TKVe_ketThuc.getDate());
		...
		if (!formattedNgayBatDau.isEmpty() && !formattedNgayKetThuc.isEmpty()) {
			...
			lbl_slvb_Ve.setText(String.valueOf(slvb));
			lbl_sltv_Ve.setText(String.valueOf(sltv));
		} else {
			JOptionPane.showMessageDialog(null, "Vui lòng chọn ngày bắt đầu và ngày kết thúc.");
		}
	}
				...
				if (cthd != null) {
					...
					for (Ve ve : listVe) {
						...
						if (!processedChuyenTau.contains(chuyenTau)) {
							...
							doanhThuTheoChuyenTau.put(chuyenTau,
									doanhThuTheoChuyenTau.getOrDefault(chuyenTau, 0f) + doanhThu);
	...
	public void createJFreeChartThongKeDoanhThu() {
		...
		JFreeChart chart = ChartFactory.createBarChart(
				"Thống kê doanh thu theo khuyến mãi", // Sửa tiêu đề
				"Đối tượng bán",
				"Doanh thu",
				dataset);
	...
	// === HÀM MỚI: TẠO CHART CHO TAB VÉ ===
	public void createJFreeChartThongKeVe() {
		// Khởi tạo dữ liệu (ví dụ: thống kê vé theo hạng)
		DefaultCategoryDataset datasetVe = createDatasetVe();
		// Create chart
		JFreeChart chartVe = ChartFactory.createBarChart(
				"Thống kê số lượng vé bán theo hạng",
				"Hạng vé",
				"Số lượng",
				datasetVe,
				PlotOrientation.VERTICAL,
				false, // Không cần legend
				true,
				false);

		// Giảm kích cỡ chữ cho tiêu đề
		chartVe.getTitle().setFont(new Font("Arial", Font.PLAIN, 16));
		// Tạo ChartPanel
		chartPanel_Ve = new ChartPanel(chartVe);
		chartPanel_Ve.setBounds(0, 0, 1311, 308);
		jp_thongKe_Ve.setLayout(null);
		jp_thongKe_Ve.removeAll();
		jp_thongKe_Ve.revalidate();
		jp_thongKe_Ve.repaint();
		// Thêm ChartPanel vào jp_thongKe_Ve
		jp_thongKe_Ve.add(chartPanel_Ve);
	}
	...
	public void createJFreeChartThongKeChuyenTau() {
		...
		JFreeChart chartCT = ChartFactory.createBarChart(
				"Thống kê doanh thu theo chuyến tàu", // Sửa tiêu đề
				"Chuyến tàu",
				"Doanh thu",
				datasetCT);
	...
	private DefaultCategoryDataset createDatasetDoanhThu() {
		...
		if (dateChooser_TKDT_batDau.getDate() == null || dateChooser_TKDT_ketThuc.getDate() == null) {
			return dataset; // Trả về dataset rỗng nếu chưa chọn ngày
		}
	...
	// === HÀM MỚI: TRUY VẤN DỮ LIỆU CHO CHART VÉ (THỐNG KÊ VÉ THEO HẠNG) ===
	private DefaultCategoryDataset createDatasetVe() {
		dataset = new DefaultCategoryDataset();

		if (dateChooser_TKVe_batDau.getDate() == null || dateChooser_TKVe_ketThuc.getDate() == null) {
			return dataset; // Trả về rỗng nếu chưa chọn ngày
		}
		...
		Map<String, Integer> soLuongVeTheoHang = new HashMap<>();
		ArrayList<HoaDon> listHD = dsHD.getHoaDonTheoNgayLapHD(formattedNgayBatDau, formattedNgayKetThuc);
		...
		for (HoaDon hd : listHD) {
			...
			if (cthd != null) {
				...
				for (Ve ve : listVe) {
					String hang = ve.getHang();
					soLuongVeTheoHang.put(hang, soLuongVeTheoHang.getOrDefault(hang, 0) + 1);
				}
			}
		}

		// Thêm dữ liệu vào dataset
		for (Map.Entry<String, Integer> entry : soLuongVeTheoHang.entrySet()) {
			String hang = entry.getKey();
			Integer soLuong = entry.getValue();
			dataset.addValue(soLuong, "Số Lượng", hang);
		}
		...
		for (HoaDon hd : listHD) {
			...
			if (cthd != null) {
				...
				for (Ve ve : listVe) {
					String chuyenTau = ve.getChuyenTau().getMaTau(); // Lấy mã chuyến tàu
					doanhThuTheoChuyenTau.put(chuyenTau,
							doanhThuTheoChuyenTau.getOrDefault(chuyenTau, 0f) + ve.tinhGiaVe());
				}
			}
		}
	...
	public void kiemTraQuyen(TrangChu_GUI trangChu) {
		...
		if (nv != null) {
			...
			if (tk != null) {
				if (tk.getPhanQuyen() == 2) { // Nếu là nhân viên
					tabbedPane.setSelectedIndex(0); // Chọn tab 0 (Theo ca)
					tabbedPane.setEnabledAt(1, false); // Vô hiệu hóa tab 1 (Doanh thu)
					tabbedPane.setEnabledAt(2, false); // Vô hiệu hóa tab 2 (Vé)
					tabbedPane.setEnabledAt(3, false); // Vô hiệu hóa tab 3 (Chuyến tàu)
				} else { // Nếu là quản lý
					tabbedPane.setSelectedIndex(0);
					tabbedPane.setEnabledAt(1, true); // Bật tab 1
					tabbedPane.setEnabledAt(2, true); // Bật tab 2
					tabbedPane.setEnabledAt(3, true); // Bật tab 3
				}
			}
		}
	}
	...
	public void xuatExcelTheoCa(ArrayList<Ve> data, NhanVien nv, LocalDateTime thoiGianBatDauCa,
			LocalDateTime thoiGianKetThucCa, JLabel label1, JLabel label2, JLabel label3)
		...
		String[] headerData_ThongTinNV = { "Mã nhân viên", "Họ Tên", "Ca làm", "Thời gian vào ca",
		...
		String[] headerData = { "STT", "Tên khách hàng", "Ga đi", "Ga đến", "Hạng", "Khuyến mãi", "Toa", "Ghế", "Mã vé",
				"Mã chuyến tàu", "Ngày đi", "Giờ đi", "Trạng thái", "Chi tiết" };
	...
	public void xuatExcelDoanhThu(ArrayList<Ve> data, NhanVien nv, JLabel label1, JLabel label2, JLabel label3)
			throws FileNotFoundException, java.io.IOException {
		...
		// Dữ liệu bảng vé
		int stt = 1;
		int rowIndex = startRow + 1; // Biến đếm dòng cho excel
		DateTimeFormatter timeFormatter = DateTimeFormatter.ofPattern("HH:mm");
		DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
		for (int i = 0; i < data.size(); i++) {
			Ve ve = data.get(i);
			...
			Row row = sheet.createRow(rowIndex++); // Sử dụng rowIndex và tăng nó
			KhachHang kh = dsKH.getKhachHangTheoMaKH(ve.getKhachHang().getMaKH());
			String[] cellData = {
					String.valueOf(stt++),
					kh.getTenKH(),
	...
	public void xuatExcelChuyenTau(ArrayList<Ve> data, NhanVien nv, JLabel label1, JLabel label2, JLabel label3)
			throws FileNotFoundException, java.io.IOException {
		...
		Map<String, Float> sortedDoanhThuTheoTau = doanhThuTheoTau.entrySet()
				.stream()
				.sorted(Map.Entry.<String, Float>comparingByValue().reversed()) // So sánh gặp theo cặp key-value(so
																				// sánh doanh thu). reversed() Đảo ngược
				...
				.collect(Collectors.toMap(
						...
						(e1, e2) -> e1, // Xử lý xung đột (nếu xảy ra). Ở đây, chỉ giữ entry đầu tiên. Nếu có hai giá
										// trị trùng key (không xảy ra trong trường hợp này), nó sẽ giữ lại giá trị đầu
										// tiên.
	...
	public void hienThiThongKeChuyenTau() {
		tabbedPane.setSelectedIndex(3); // Cập nhật index
	}
	...
	// === HÀM MỚI: CHỈ ĐỊNH TAB VÉ ===
	public void hienThiThongKeVe() {
